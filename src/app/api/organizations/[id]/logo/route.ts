...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  let newUrl: string;
  try {
    newUrl = await saveUploadedImage(file, `organizations/${params.id}`);
  } catch (e) {
  ...
  // through never leaves the organization without any logo.
  const previous = await prisma.organization.findUnique({ where: { id: params.id }, select: { logoUrl: true } });
  ...
  return NextResponse.json({ ok: true, logoUrl: organization.logoUrl });
}
