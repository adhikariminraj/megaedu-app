...
/**
 * Edit and/or deactivate an Organization Event — Organization Admin
 * only. Mirrors schools/[id]/events/[eventId]/route.ts exactly,
 * including the "no DELETE" convention — deactivation (isActive:
 * false) is how a wrongly-created or cancelled Event is removed, same
 * as School Events. Re-verifies the Event's own organizationId against
 * the URL's id before allowing any change.
 */
export async function PATCH(req: NextRequest, { params }: { params: { id: string; eventId: string } }) {
  ...
  if (!event || event.organizationId !== params.id) {
