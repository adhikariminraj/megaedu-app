...
export default function OrganizationLogoManager({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch(`/api/organizations/${organizationId}/logo`, { method: "POST", body: formData });
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-5">
      <h3 className="font-semibold text-slate-800 mb-1">Organization Logo</h3>
      <p className="text-xs text-slate-400 mb-4">
        Shown on the Organization Directory card and provider profile page. PNG, JPEG, or WebP, up to 2MB.
      </p>

      <div className="flex items-center gap-4">
        <Avatar src={preview || logoUrl} name={organizationName} variant="school" size="xl" />
