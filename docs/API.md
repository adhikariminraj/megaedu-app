...
| `DELETE` | `/api/schools/[id]/grades/[schoolGradeId]/subjects/[gradeSubjectId]` | Remove one subject from a grade's offering | — | Only an **empty** offering: `409` with a readable list if anything depends on it (teacher assignments, plans, units, homework, evaluations, meetings, framework overrides, results, publications), `409` for a closed session's offering, `409` "already removed" if a simultaneous request removed it first (finding F8) |
| `POST` | `/api/schools/[id]/teacher-academic-assignments` | Bulk-create teacher subject-teaching assignments for one session | `{academicSessionId, assignments: [{teacherId, schoolGradeId, sectionId?, subjectId}]}` | `sectionId: null` = grade-wide; subject must be offered at that grade this session (via `GradeSubject`) or the item is skipped; the same teacher can't hold both a grade-wide and section-specific row for the same subject/grade/session (skipped, not errored) — see [PRODUCT_RULES.md](PRODUCT_RULES.md). Runs as one `SERIALIZABLE` transaction (F2 fix, rule A1); when a simultaneous request conflicts, the whole batch rolls back and the route returns `409` ("please refresh and try again") |
...
**A deadlock or serialization failure (finding F3)**: the bulk routes — attendance, class-teacher assignments, grade placements, grade sections, grade subjects, school subjects, teacher-grade assignments — and teacher academic assignments answer `409` with their usual "… please refresh and try again" message when PostgreSQL aborts their transaction as a deadlock victim or for a serialization failure; the batch has rolled back whole, so retrying is safe.

**A just-removed offering (finding F8)**: the write routes that reference a subject offering — teacher academic assignments, teaching plan, units, homework, meetings, evaluations, framework assignments, result entry and publish — answer `409` "This subject is no longer offered at this grade this session — please refresh." when the offering was removed by a simultaneous request (a `gradeSubjectId` foreign-key failure, `P2003`), instead of a raw `500`.
...
| Method | Endpoint | Purpose | Important request data | Important response / errors |
|---|---|---|---|---|
| `POST` | `/api/schools/[id]/class-teacher-assignments` | Bulk-create Grade Coordinator / Class Teacher assignments | `{academicSessionId, assignments: [{teacherId, schoolGradeId, sectionId?}]}` | No overlap rule (grade-wide and section-specific may coexist); an already-filled slot is silently skipped; a request that loses a simultaneous race for the same slot gets `409` and none of its batch is saved |
...
| `POST` | `/api/schools/[id]/grades/[schoolGradeId]/subjects/[gradeSubjectId]/teaching-plan` | Set or update the planned-total/display-label plan for one scope | `{sectionId?, plannedTotal, unitLabel?}` | Find-or-update-else-create — never a duplicate row per `(gradeSubjectId, sectionId)`; a grade-wide plan is also protected by a partial unique index (F2 fix, rule A2). When simultaneous saves collide, the save whose insert is rejected updates the plan the other created and returns `200` (before: a raw `500` for a section plan, a second row for a grade-wide one); no ordering between simultaneous saves is defined |
...
| Method | Endpoint | Purpose | Important request data | Important response / errors |
|---|---|---|---|---|
| `POST` | `/api/schools/[id]/students/[studentId]/evaluations` | Create a General (`gradeSubjectId` omitted) or Subject (`gradeSubjectId` set) qualitative evaluation | `{teacherId?, gradeSubjectId?, remarks}` | Auth: `requireTeacherAssignment` (subject set) or `requireClassTeacher` (general) for the acting teacher, OR `requireSchoolAdmin` with a validated `teacherId` in the body. `409` if this teacher already has an evaluation for this student/session/scope — explicit app-level pre-check for the `gradeSubjectId: null` NULL-uniqueness gap, plus (F2 fix, rule A3) the database for a simultaneous duplicate — the `@@unique` for a subject evaluation, a partial unique index for a general one — which gets the same `409` (before: a raw `500`, or a second general evaluation) |
