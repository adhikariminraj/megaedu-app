...
- **`CoScholasticResult`** — one student's grade for one area, for one scope. `coScholasticPeriodId` has exactly one meaning (`null` = annual, a real id = that period), the same discriminator idiom `AssessmentComponent.periodId` already uses. Inherits the same documented SQLite NULL≠NULL gap as `AssessmentComponent`/`AssessmentFrameworkAssignment`/`StudentEvaluation` — closed via an explicit pre-check in `upsertCoScholasticResult()`, not a new problem. PostgreSQL treats `NULL`s the same way (kept deliberately, decision D7), so the pre-check still applies there — but an application pre-check is not safe against truly simultaneous requests (finding F2, not fixed; see [KNOWN_GAPS.md](KNOWN_GAPS.md#postgresql-findings-f1f7)).
