...
**Why**: SQLite's Prisma connector doesn't support native enums, even unused ones — this broke a migration once ("this bit us once already in Phase 1") and the rule was carried forward explicitly into Phase 2's design brief. The development database is now PostgreSQL (branch `pg-foundation`), which does support enums; the plain-`String` rule is kept deliberately as a convention, not re-opened.
...
### Bulk writes use one transaction, not a loop of individual commits ✅ (duplicate handling made database-neutral in PG-KM2)
**Rule**: a bulk write (many rows, each individually validated/skipped) runs as one `prisma.$transaction(async (tx) => {...})`, not a bare `for` loop of unwrapped `prisma.create()` calls. Inside the transaction, duplicates are detected **before** the write — rows that already exist, and items repeated within the same batch, are pre-checked and counted as `skipped` — and a unique-constraint error (`P2002`) is never caught and continued inside the transaction. The route wraps the whole transaction in an outer handler: a `P2002` there can only mean a concurrent request wrote the same row first, so the whole batch rolls back and the route returns `409` ("please refresh and try again"). Since the F3 fix (2026-09-26) the same `409` answers a transaction that PostgreSQL aborted as a deadlock victim or for a serialization failure — recognised by `isTransactionConflict()` (`src/lib/dbErrors.ts`), because Prisma 5.20 leaves a deadlock unmapped. Measured directly (before PG-KM2): on `grade-placements`/`teacher-assignments`, 200 sequential unwrapped creates took ~15.3s (~77ms/row) vs. ~177ms (~0.9ms/row) wrapped in one transaction — ~86x faster. The same pattern was built transactional *from the start* for `grade-decisions` (100 students, 328ms real HTTP call) and the session rollover sweep (120 students, 365ms real HTTP call).
**Resolved (PG-KM2, 2026-09-25, commit `60b23b5`)** — the routes originally used a per-item try/catch inside the transaction that caught `P2002` and continued. PG-KM2 replaced that with the pre-check + outer `409` pattern above and was verified on SQLite; compatibility test T07 then confirmed on a real PostgreSQL 18 instance that catch-and-continue inside a transaction fails; the reworked routes were exercised against PostgreSQL in PG-KM8 and PG-KM9. See [CHANGELOG.md](CHANGELOG.md). Kept here as the record of why:
**Why this needed revisiting before Postgres** (original text): the per-item try/catch relies on a caught statement error (e.g. a unique-constraint violation) *not* poisoning the rest of the transaction — verified true on SQLite (a deliberate collision-then-continue test still committed the row that came after the collision). **This is not true on Postgres**: there, any failed statement aborts the whole transaction until an explicit `ROLLBACK`/`SAVEPOINT`, so every subsequent `tx.create()` call would itself start failing and get miscounted as `skipped` even when it was actually valid. Since `.env.example`/`schema.prisma` both mark Postgres as the intended production target, **this pattern must be re-checked (likely reworked to pre-filter duplicates before the transaction) before that migration**. Full list of affected routes in [KNOWN_GAPS.md](KNOWN_GAPS.md).
**Applies to** (current, after PG-KM2): `POST /api/schools/[id]/grade-placements` and `POST /api/schools/[id]/teacher-assignments` (the two routes named in the original text), plus the other bulk routes reworked in PG-KM2 — attendance, class-teacher assignments, teacher academic assignments, grade sections, grade subjects and school subjects — and the rollover carry-forward sweep, `carryForwardEligibleStudents()` (`src/lib/gradeRollover.ts`). The original text listed the sweep as not applicable; in fact it also caught `P2002` and continued inside its transaction, and PG-KM2 replaced that with an up-front "already has a row in the target session" skip and no catch. `grade-decisions` validates eligibility *before* opening its transaction and catches no constraint error inside it, so PG-KM2 did not change it.
...
**Rule**: `TeacherAcademicAssignment.sectionId: null` means grade-wide (every section); a real value means one specific section. For the SAME `(teacherId, academicSessionId, schoolGradeId, subjectId)` tuple, a grade-wide request is rejected if any row already exists for that tuple, and a section-specific request is rejected only if a grade-wide row already exists for it. Enforced in the route (`POST /api/schools/[id]/teacher-academic-assignments`) — SQL unique indexes treat `NULL ≠ NULL`, so a plain `@@unique` cannot catch two grade-wide rows colliding, and no index can express the grade-wide vs section-specific rule. **Update (F2 fix, rule A1, 2026-09-26)**: on PostgreSQL the route check alone was not safe under simultaneous requests; the route's transaction now runs at `SERIALIZABLE` isolation (the separate justification decision D8.8 requires), and the partial unique index `TeacherAcademicAssignment_one_grade_wide_per_teacher_subject` also blocks a second grade-wide row; a request that loses to a simultaneous one gets `409` and its batch rolls back.
...
### Grade Coordinator/Class Teacher assignment allows grade-wide and section-specific to coexist — the opposite of Phase 3A's overlap rule ✅
**Rule**: `ClassTeacherAssignment` has no overlap-blocking rule between a grade-wide (Grade Coordinator) and section-specific (Class Teacher) row for the same grade — both may exist simultaneously (e.g. a grade-wide coordinator plus a Class Teacher for each individual section). Uniqueness is on the *slot* (`schoolGradeId, sectionId, academicSessionId`), not the teacher, so at most one Grade Coordinator/Class Teacher per slot, per session — but different slots (the whole grade, and each of its sections) can each have their own holder at once.
...
**Rule**: `@@unique([schoolGradeId, sectionId, academicSessionId])` reliably blocks a duplicate section-specific slot but not a duplicate grade-wide (`sectionId: null`) one, so the create route pre-checks grade-wide requests explicitly, inside the same transaction, before relying on the DB constraint at all. On PostgreSQL that pre-check alone is not safe under simultaneous requests (finding F1), so since PG-F1 (branch `pg-foundation`) the grade-wide slot is also enforced by the partial unique index `ClassTeacherAssignment_one_grade_wide_per_session`; a request that loses such a race gets `409` and its whole batch rolls back.
**Why**: this is the exact same `NULL ≠ NULL` unique-index behavior already documented for `TeacherAcademicAssignment`'s overlap rule — but it was still missed on first pass for `ClassTeacherAssignment` and only caught by a live duplicate-creation test during Phase 3B verification (a second grade-wide Grade Coordinator was wrongly accepted). Recorded here explicitly as a reminder: **any new model with an optional `sectionId` in its unique key needs this same app-level check**, not just the two places it's been caught so far.
...
**Rule**: `StudentEvaluation.gradeSubjectId` being `null` (General — Grade Coordinator/Class Teacher) or set (Subject — Subject Teacher) is the *entire* distinction. No separate `evaluationType` string, no two models. The UI derives its "General Student Evaluation" / "Subject Evaluation" label purely from whether the field is set.
...
**Rule**: `@@unique([studentId, teacherId, academicSessionId, gradeSubjectId])` reliably blocks an exact duplicate subject-specific slot, but the same `NULL ≠ NULL` unique-index gap already found in `TeacherAcademicAssignment` and `ClassTeacherAssignment` means it does **not**, by itself, block a second general (`gradeSubjectId: null`) evaluation from the same teacher/student/session. The create route pre-checks this exact case explicitly, from the first implementation (the pre-check runs without a transaction; an earlier version of this rule said "inside its transaction"). **Update (F2 fix, rule A3, 2026-09-26)**: on PostgreSQL the pre-check alone was not safe under simultaneous requests; the partial unique index `StudentEvaluation_one_general_per_student_teacher_session` now enforces the rule in the database, and a simultaneous duplicate gets the same `409`.
...
**Applies to**: `AssessmentFrameworkAssignment`, `AssessmentComponent`. Verified live: a duplicate grade-default assignment, a duplicate subject-override assignment, and a duplicate framework-level component each returned `409`; two identically-named, period-less components submitted together in one framework-creation request returned `400` before any row was created. **Update (F2 fix, 2026-09-26)**: on PostgreSQL a pre-check alone is not safe under simultaneous requests (finding F2); the grade-default case is now also enforced by the partial unique index `AssessmentFrameworkAssignment_one_default_per_grade_session`. The framework-level component case (`AssessmentComponent`, F2 rule A4) is also enforced, since the same day, by the partial unique index `AssessmentComponent_unique_name_framework_level` (migration `4_f2_a1_to_a4_unique`); adds and renames return `409` for a simultaneous duplicate too.
...
**Rule**: `requireClassTeacher()` is never composed into any Phase 3D-2/3/4 write route. A Grade Coordinator/Class Teacher's involvement in assessment results is exactly the same as any other teacher's — only via `requireTeacherAssignment()` if they separately hold a subject assignment.
...
### `CURRENT_ROSTER_STATUSES` defines who counts as "currently in a grade" — and it's not `ENROLLED` alone ✅
**Rule**: `CURRENT_ROSTER_STATUSES` (`src/lib/gradeHistory.ts`) — `["ENROLLED", "COMPLETED", "REPEATED"]` — is the single, shared definition of "physically present in this grade for the rest of the current session," used identically by the Class Overview roster (`/dashboard/grades/[schoolGradeId]`) and the Grades index's own "N enrolled" card count (`/dashboard/grades/page.tsx`). Only `TRANSFERRED`/`LEFT` are excluded.
**Why**: a decision about *next* session (`COMPLETED` = promoted, `REPEATED` = repeating) doesn't remove a student from *this* grade the moment it's recorded — the school year isn't over, and the student is still sitting in class. The original `ENROLLED`-only query made a genuinely real seed-data student (already decided `REPEATED` on their own current-session row, mid-year) invisible on a page meant to show who's currently in the grade — caught live, not assumed, and fixed by broadening the definition rather than special-casing the one row. `TRANSFERRED`/`LEFT` stay excluded because those students have genuinely left the school — there's no "rest of this session" to count them in.
**Applies to**: `src/lib/gradeHistory.ts`, `/dashboard/grades/[schoolGradeId]/page.tsx`, `/dashboard/grades/page.tsx`. Verified live: the Grades index card and the Class Overview page produced the identical count (4) for a mix of a fresh admission, a genuinely repeated student, an unassigned student, and Demo Student's own already-`REPEATED` current-session row — with a separately-created `TRANSFERRED` student correctly excluded from both.

### The Promotion action panel's own eligibility is deliberately narrower than the Class Overview's display roster ✅
**Rule**: `CURRENT_ROSTER_STATUSES` governs what the Class Overview *shows*; it does not change what the Promote/Repeat/Transfer/Leave panel can *act on*. Only a still-`ENROLLED` `GradeHistory` row is eligible for a new decision — enforced independently, server-side, by `POST /api/schools/[id]/grade-decisions` itself (`recordGradeDecision()`'s caller re-validates `status === "ENROLLED"` and silently skips anything else, counted in `skipped`, never an error).
**Why**: these are two different questions — "who is currently in this grade" (display) vs. "who can legitimately receive a *new* decision right now" (action eligibility) — and conflating them would either hide already-decided-but-still-present students from the roster entirely (the original bug) or make it look like re-deciding an already-decided student is a normal, expected action. Broadening the display roster required no change to the decision route at all; its existing server-side guard already made the broadening safe.
**Applies to**: `/dashboard/grades/[schoolGradeId]`, `POST /api/schools/[id]/grade-decisions`. The separate "Assign Section" panel on the same page is *not* narrowed this way — section reassignment has nothing to do with promotion eligibility, so an already-decided student can still have their section corrected through it.

### Top 5 ranking on the Class Overview is grade-wide, computed once, never re-scored per section ✅
**Rule**: ranking is computed across the entire grade's roster before any section-based grouping happens for display — a student in Section A and a student in Section D can legitimately hold Rank 1 and Rank 2 on the same page. The ranking basis and numbers come entirely from the existing central calculation engine (`fetchAssessmentResults()`, `computeUnweightedGPA()`, `computeUnweightedAveragePercentage()` — see [ASSESSMENT_RESULTS.md](ASSESSMENT_RESULTS.md)), filtered to published results only; nothing is duplicated or recalculated on this page.
**Why**: explicitly required — a school wants one overall Class 9 ranking, not five independent per-section leaderboards, while still being able to view students grouped by section for day-to-day use. Section grouping and ranking are genuinely orthogonal concerns, so keeping them computed independently (rank first, group for display second) avoids conflating them.
**Applies to**: `/dashboard/grades/[schoolGradeId]`. Verified live: a Section D student (Demo Student, 90.0%) held Rank 1 while a Section A student (60.0%) held Rank 2, on the same page, in the same request.
