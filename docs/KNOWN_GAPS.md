...
Recorded during the PostgreSQL block. **F1, F2 and F8 are fixed on branch `pg-foundation` (F1 in PG-F1, 2026-09-25; F2's rules A6 and A5, then A1–A4, and F8 on 2026-09-26); F3–F7 are not fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10`, `PG-F1`, `PG-F2`, `PG-F2FIX`, `PG-F2FIX2`, `PG-F8` and `PG-F8FIX` on the development machine (see [TESTING.md](TESTING.md)).
...
| F3 | Low ⚠️ | Bulk routes do not map Prisma `P2034` (deadlock / serialization failure) — one would surface as a `500`. PG-KM9 observed **0 deadlocks** (including the reversed-order probe W3b). | Map `P2034` to `409`/`503`; optionally a consistent insert order. (Since the F2 fix, the teacher-academic-assignments route already maps `P2034` to `409`, because its transaction runs at `SERIALIZABLE` isolation — see F2; the other bulk routes still do not.) |
