...
Recorded during the PostgreSQL block. **F1, F2, F3 and F8 are fixed on branch `pg-foundation` (F1 in PG-F1, 2026-09-25; F2's rules A6 and A5, then A1–A4, F8 and F3 on 2026-09-26); F4–F7 are not fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10`, `PG-F1`, `PG-F2`, `PG-F2FIX`, `PG-F2FIX2`, `PG-F8`, `PG-F8FIX` and `PG-F3FIX` on the development machine (see [TESTING.md](TESTING.md)).
...
| F4 | Info | Losers of simultaneous identical actions get a `409` asking the user to refresh (e.g. attendance, rollover, affiliation, submission/review numbering). Behavior is correct; only the wording needs product confirmation. | — |
| F5 | Info | The homework-completion save concurrency limit of 5 is safe on PostgreSQL: 44 benchmark runs, 1,860 operations, 0 failures. | Keep 5. |
| F6 | Info | Test limitations: PG-KM9 ran against the development server, each race once (timing-dependent races can pass one run and fail another), and the connection sampler polled every 250 ms. | — |
| F7 | Low | **Fresh seed vs `db:verify:demo` mismatch — a demo-data/verification consistency finding, not a PostgreSQL defect.** A fresh `seed-demo.ts` intentionally leaves **2** Class 9 students without a section; the current dataset (the SQLite `dev.db` and `megaedu_dev`, which is its copy) has **1**. A read-only audit query found a later, audited reassignment of Kalpana Thapa from no section to Section B. The audit trail records a later explicit reassignment that explains the observed current state, while the verifier retains a historical comment attributing the difference to RNG non-determinism. The two explanations are not reconciled in this change. `verify-demo-data.ts` checks the later/current state, so a rebuild from seeds (runbook RB2) passes **16 of 18 checks** while the current dataset passes 18 of 18; the PostgreSQL rebuild itself succeeds. The same mismatch is expected on SQLite, because the mismatch is between the seed output and the verification expectations, not PostgreSQL behavior (inferred, not tested). See [DEMO_DATA.md](DEMO_DATA.md#verifying-the-demo-data). | Documentation-only treatment for now; code/verification fix remains undecided and unapproved. |
