...
Recorded during the PostgreSQL block. **F1 is fixed on branch `pg-foundation` (PG-F1, 2026-09-25), and so are F2's rules A6 and A5 (2026-09-26); F2's rules A1–A4 and F3–F7 are not fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10`, `PG-F1`, `PG-F2` and `PG-F2FIX` on the development machine (see [TESTING.md](TESTING.md)).
