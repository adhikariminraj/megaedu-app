...
Recorded during the PostgreSQL block. **None is fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9` and `PG-KM10` on the development machine (see [TESTING.md](TESTING.md)).

| # | Severity | Finding (what was observed) | Proposed fix — not approved |
|---|---|---|---|
| F1 | High ⚠️ | **Grade-wide Class Teacher slot race.** PG-KM9 race W3d: 10 simultaneous requests for one free grade-wide `ClassTeacherAssignment` slot all returned `200` and created **10 rows** for that slot (expected 1). The "one grade-wide Class Teacher per grade per session" rule is checked only by the application; the unique key cannot cover it because `sectionId` is `NULL` and PostgreSQL treats `NULL`s as distinct (kept deliberately, decision D7). | A partial unique index on `(schoolGradeId, academicSessionId) WHERE "sectionId" IS NULL` (an explicit exception to D7), or a transaction-scoped advisory lock around the check-and-insert. |
