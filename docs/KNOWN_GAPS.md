...
Recorded during the PostgreSQL block. **F1 is fixed on branch `pg-foundation` (PG-F1, 2026-09-25); F2–F7 are not fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10` and `PG-F1` on the development machine (see [TESTING.md](TESTING.md)).
...
| F2 | Medium ⚠️ (risk) | **Other application-only rules have the same shape** and are not concurrency-safe in principle: `TeacherAcademicAssignment` grade-wide vs section overlap (race W3e did **not** reproduce a violation in its one run), the general `StudentEvaluation`, the annual `CoScholasticResult`, and `AssessmentComponent`/`AssessmentFrameworkAssignment` rows with `NULL` scope. | Advisory lock, or `SERIALIZABLE` with retry (decision D8.8), per rule. |
