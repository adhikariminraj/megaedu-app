...
> Last verified: 2026-09-24 for the Organization/public-visibility entries below (Organization-strengthening 10-km block — owner-eligibility rule on public listings, public link safety, directory search, profile self-editing); all other entries as of 2026-09-10 (K1-K8 reconciliation — public Organization provider profile, Academy participation/visibility, Opportunity edit/delete, Course↔Provider reciprocal links, MEGA Academy navigation label, Organization logo management, obsolete certificate backfill script removal, Organization Events & Resources; plus a MEGA Academy planning-scope reconciliation adding the K10-K17+ development track and Development Gate mapping) — every item below was actively re-checked against the current codebase before being listed (grep/read, not assumption). If an item is ever fixed, move it out of this file rather than leaving it marked open.

## Data model gaps

### `School.isActive` / `Organization.isActive` are read but never written ⚠️
Both fields default to `true` and are read as a public-visibility filter across the directories and listings (`/schools`, `/organizations`, both detail pages, `/courses`, the homepage, `/opportunities`, `/resources`, `/approaches`, and Academy enrollment) plus the Platform Admin dashboard counts, but **no route anywhere ever sets either to `false`**. There is no deactivation action in the app, so every `isActive` check is currently inert — correct and ready, but never triggered. (Re-verified 2026-09-24; the earlier "exactly three files" count predates the K2B/K1 visibility work and is no longer accurate.)

## Database portability

> Database entries re-verified 2026-09-25 (PostgreSQL 10-km block PG-KM1–PG-KM10, branch `pg-foundation`, not merged into `main`).

**Resolved — SQLite-specific transaction behavior in bulk-write routes.** This entry previously recorded that `grade-placements` and `teacher-assignments` caught a unique-constraint violation inside an open transaction and carried on — safe on SQLite, but PostgreSQL aborts the rest of the transaction. PG-KM2 (commit `60b23b5`, on `main`; verified on SQLite) reworked those routes, together with the other duplicate/concurrency handlers that audit found (attendance, class-teacher and academic assignments, sections, subjects, homework, rollover). Compatibility test T07 then confirmed on a real PostgreSQL 18 instance that catching a constraint error and continuing inside a transaction fails, and the reworked routes were exercised against PostgreSQL in PG-KM8 and PG-KM9. See [CHANGELOG.md](CHANGELOG.md) and [PRODUCT_RULES.md](PRODUCT_RULES.md).

### PostgreSQL findings (F1–F8)

Recorded during the PostgreSQL block. **All eight findings are resolved on branch `pg-foundation`: F1, F2, F3, F4, F7 and F8 are fixed (F1 in PG-F1, 2026-09-25; F2's rules A6 and A5, then A1–A4, F8, F3, F4 and F7 on 2026-09-26), and F5 and F6 are closed as informational.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10`, `PG-F1`, `PG-F2`, `PG-F2FIX`, `PG-F2FIX2`, `PG-F8`, `PG-F8FIX`, `PG-F3FIX` and `PG-MP1` on the development machine (see [TESTING.md](TESTING.md)).

| # | Severity | Finding (what was observed) | Proposed fix — not approved unless marked |
|---|---|---|---|
| F1 | High — ✅ **fixed (PG-F1)** | **Grade-wide (Grade Coordinator) slot race.** PG-KM9 race W3d: 10 simultaneous requests for one free grade-wide `ClassTeacherAssignment` slot all returned `200` and created **10 rows** for that slot (expected 1). The "at most one Grade Coordinator (grade-wide row) per grade per session" rule was checked only by the application; the existing unique key cannot cover it because `sectionId` is `NULL` and PostgreSQL treats `NULL`s as distinct (kept deliberately, decision D7). | **Implemented** (approved 2026-09-25 as an explicit exception to D7; branch `pg-foundation` only): migration `2_class_teacher_grade_wide_unique` — partial unique index `ClassTeacherAssignment_one_grade_wide_per_session` on `(schoolGradeId, academicSessionId) WHERE "sectionId" IS NULL`. Verified: the race reproduced on a restored copy went from 10 rows to exactly 1 (9 rejected by the index); through the app, 3 simultaneous losing requests got `409` with their whole batches rolled back; data, integrity suite and `db:verify:demo` unchanged. (The alternative, an advisory lock, was not chosen.) |
| F2 | Medium — ✅ **fixed (A1–A6)** | **Other application-only rules with the same shape.** The F2 investigation (2026-09-26, evidence `PG-F2`) reproduced duplicates for all six on a throwaway copy, with 10 simultaneous requests that all read before any wrote: **A1** `TeacherAcademicAssignment` (10 grade-wide rows; and 5 grade-wide + 5 section rows for one teacher and subject — the overlap PG-KM9 race W3e had not reproduced), **A2** `TeachingPlan` (10 grade-wide plans; not in the original list), **A3** `StudentEvaluation` (10 general evaluations), **A4** `AssessmentComponent` (10 framework-level adds; 5 renames to one name), **A5** `AssessmentFrameworkAssignment` (10 grade defaults), **A6** `CoScholasticResult` (10 annual rows holding conflicting grades). No duplicates existed in the data. A2–A4 also returned a raw `500` when a section, subject or period slot collided. The investigation also found that the annual co-scholastic save ignored the academic session, so a save in a later session would have overwritten an earlier session's annual grade. | **A6 and A5 implemented** (approved 2026-09-26 as two explicit exceptions to D7): migration `3_f2_annual_and_default_unique` adds the partial unique indexes `CoScholasticResult_one_annual_per_student_area_session` and `AssessmentFrameworkAssignment_one_default_per_grade_session`; the annual co-scholastic save is scoped to the session and, when a simultaneous save created the row first, updates it — the row ends up holding one of the submitted values (no ordering between simultaneous saves is defined); the framework-assignment route returns `409` for any duplicate. Rehearsed on a restored copy, then applied to the development database and verified through the app (evidence `PG-F2FIX`; details in [TESTING.md](TESTING.md)). **A1–A4 implemented** (approved 2026-09-26 as four explicit exceptions to D7): migration `4_f2_a1_to_a4_unique` adds the partial unique indexes `TeacherAcademicAssignment_one_grade_wide_per_teacher_subject`, `TeachingPlan_one_grade_wide_per_grade_subject`, `StudentEvaluation_one_general_per_student_teacher_session` and `AssessmentComponent_unique_name_framework_level`. A1's grade-wide vs section overlap spans different rows, which no index can express, so the teacher-academic-assignments route's transaction runs at `SERIALIZABLE` isolation (the separate justification decision D8.8 requires; on the restored copy the same race at the default isolation level still produced the overlap with the new index in place); the route maps a serialization failure (`P2034`) or a duplicate (`P2002`) to `409`. A2: when simultaneous saves collide, the save whose insert is rejected updates the plan the other created — the plan ends up holding one of the submitted values (no ordering between simultaneous saves is defined). A3 and A4 (add and rename): `409` with the routes' existing messages. Rehearsed on a restored copy, then applied to the development database and verified through the app, including forced-conflict tests (evidence `PG-F2FIX2`; details in [TESTING.md](TESTING.md)). The four other nullable unique keys the investigation covered (the student admission number and the three `Address` owner keys) allow several `NULL` rows by design. |
| F3 | Low — ✅ **fixed (PG-F3FIX)** | **A bulk write chosen as a deadlock victim answered a raw `500`.** The seven bulk routes (attendance, class-teacher assignments, grade placements, grade sections, grade subjects, school subjects, teacher-grade assignments) mapped only a duplicate (`P2002`) to `409`. PG-KM9 observed **0 deadlocks** (including the reversed-order probe W3b), so the risk was real but rare. **Correction to this finding's original wording**, which said a deadlock arrives as Prisma `P2034`: in Prisma 5.20 only a serialization failure (SQLSTATE `40001`) arrives as `P2034`; a deadlock (SQLSTATE `40P01`) arrives **unmapped**, as a `PrismaClientUnknownRequestError` — found when the fix's first step reproduced the `500` (evidence `PG-F3FIX`). The teacher-academic-assignments route's own `P2034` mapping therefore missed deadlocks too. | **Implemented** (approved 2026-09-26): a shared helper `isTransactionConflict()` (`src/lib/dbErrors.ts`) recognises a serialization failure (`P2034`) and a deadlock (an unmapped error carrying SQLSTATE `40P01`); the seven bulk routes and the teacher-academic-assignments route answer it with their existing `409` ("… please refresh and try again"), and the batch has rolled back whole. Forced deadlocks over HTTP: before the fix a raw `500`, after it `409` on all eight routes, with nothing persisted (details in [TESTING.md](TESTING.md)). A consistent insert order was not adopted (no deadlock observed in normal use). **Observations, not fixed**: `academicSessionTransitionErrorResponse()` (the academic-session routes) maps only `P2034` to `503`, so a deadlock there would still be a raw `500`; and an interactive transaction that waits longer than Prisma's default 5-second timeout fails with an unmapped `P2028` (none observed). |
| F4 | Info — ✅ **fixed (PG-MP1)** | Losers of simultaneous identical actions get a `409` asking the user to refresh (e.g. attendance, rollover, affiliation, submission/review numbering). Behavior is correct; only the wording needs product confirmation. | **Implemented** (approved 2026-09-26): an inventory of every concurrency message found one house pattern — "… was just … by someone else — please refresh and try again." — and four messages with technical wording ("concurrently" ×2 in mark-sheet issue/correction; "by another request" and "since this action was requested" in academic-session transitions), now reworded to it. No behavior changed. "At the same moment" (homework submissions and reviews, often the same person submitting twice), the academic-session `503` ("please try again in a moment") and F8's "please refresh." stay deliberately different. The pattern is recorded in [DEVELOPMENT_GUIDELINES.md](DEVELOPMENT_GUIDELINES.md). |
| F5 | Info — ✅ **closed (informational)** | The homework-completion save concurrency limit of 5 is safe on PostgreSQL: 44 benchmark runs, 1,860 operations, 0 failures. | Keep 5 — decided; no action (closed 2026-09-26). |
| F6 | Info — ✅ **closed (informational)** | Test limitations: PG-KM9 ran against the development server, each race once (timing-dependent races can pass one run and fail another), and the connection sampler polled every 250 ms. | No action (closed 2026-09-26). Later work added deterministic methods — barrier races, forced conflicts and forced deadlocks; see [TESTING.md](TESTING.md). |
| F7 | Low — ✅ **fixed (PG-MP1)** | **Fresh seed vs `db:verify:demo` mismatch — a demo-data/verification consistency finding, not a PostgreSQL defect.** A fresh `seed-demo.ts` intentionally leaves **2** Class 9 students without a section; the current dataset (the SQLite `dev.db` and `megaedu_dev`, which is its copy) has **1**. A read-only audit query found a later, audited reassignment of Kalpana Thapa from no section to Section B. The audit trail records a later explicit reassignment that explains the observed current state, while the verifier retains a historical comment attributing the difference to RNG non-determinism. The fix settled this: the audited reassignment is the explanation. `verify-demo-data.ts` checks the later/current state, so a rebuild from seeds (runbook RB2) passed **16 of 18 checks** while the current dataset passes 18 of 18; the PostgreSQL rebuild itself succeeds. The same mismatch is expected on SQLite, because the mismatch is between the seed output and the verification expectations, not PostgreSQL behavior (inferred, not tested). See [DEMO_DATA.md](DEMO_DATA.md#verifying-the-demo-data). | **Implemented** (approved 2026-09-26): `seed-demo.ts` now ends by reproducing that audited reassignment through the same path (`reassignSection()`) — the first student placed without a section, Kalpana Thapa, moves to Section B — after all other data is generated, without drawing on the seeded random-number sequence, and only while she is still unassigned. A rebuild from seeds now passes **18 of 18** (Class 9: A 9, B 9, C 8, D 8, 1 unassigned; one section-change audit row) with the rest of the demo data unchanged; the verifier's old random-number comment was replaced, its checks unchanged. The current dataset is untouched and still passes 18 of 18 (evidence `PG-MP1`). |
| F8 | High — ✅ **fixed (PG-F8FIX)** | **Removing a subject offering could silently delete or reclassify academic data.** The offering-removal route (`DELETE /api/schools/[id]/grades/[schoolGradeId]/subjects/[gradeSubjectId]`; one click, no confirmation, no audit) checked only for teacher assignments. Once those were removed and before any results were published, a removal deleted through database cascades the teaching plan, units, unit tests and **entered unit-test marks**, and **homework with its applicability, completions, the append-only completion audit log, submissions and reviews**; it turned subject evaluations and meetings into general ones and a subject framework override into the grade default (SET NULL). With results or publications present it returned a raw `500`, as it did when a SET NULL collided with an F2 index. The investigation (2026-09-26, evidence `PG-F8`) reproduced it on a throwaway copy: one removal returned `200`, deleted 36 rows and reclassified 5. No offering in the data was in that state. The API would also have accepted a closed session's offering, and a write that met a just-removed offering returned a raw `500`. | **Implemented** (approved 2026-09-26): only an **empty** offering can be removed. Migration `5_f8_offering_restrict` makes every foreign key to `GradeSubject` `RESTRICT` (the six that cascaded or set NULL; Prisma-expressible and CI-generated — no D7 exception, no drift allow-list lines). The route answers `409` with a readable list of dependants, `409` for a closed session's offering, and after a failed delete looks again (`409`, or "already removed"). The nine write routes that reference an offering map a foreign-key failure on `gradeSubjectId` (`P2003`) to `409` "no longer offered". The UI asks for confirmation. `MarkSheetSubject.gradeSubjectId` stays a plain snapshot id without a foreign key, by design. Rehearsed on a restored copy, then applied to the development database and verified through the app (evidence `PG-F8FIX`; details in [TESTING.md](TESTING.md)). |

What PostgreSQL itself proved in the same work (not findings): simultaneous identical attendance submissions (including reversed student order) and rollovers ended with one `200` and clean `409`s; both partial unique indexes (`AcademicSession_one_active_per_school`, `StudentSchoolAffiliation_one_open_per_student`) turned real races into exactly one winner; 315 HTTP requests, including 303 requests issued in concurrent bursts, produced no `5xx`, no deadlocks and no pool timeouts. Decisions still pending: production pool settings and hosting (D3). The SQLite rollback window and `dev.db` retirement (D13) and the reverse copy to SQLite (RB5, not built) are decided — see [DEPLOYMENT.md](DEPLOYMENT.md#database-rollback--recovery-runbook-).
...
Organization Resources (`POST`/`PATCH`/`DELETE /api/organizations/[id]/resources`, hard delete — Resource has no reverse relations) now exist and display publicly on `/organizations/[slug]` (capped at 5), but there is no file-upload mechanism for `Resource.fileUrl` (it remains a plain, manually-entered string, unlike `School.logoUrl`/`Organization.logoUrl`, which do have real upload infrastructure) and no subject/grade/approach-based filtering on the public `/resources` page for either School- or Organization-owned resources. (Since 2026-09-24 `/resources` does apply the owner-eligibility rule — owner must be `verified && isActive` — but it still has no search or filters.) School-side Resource posting also does not exist: `Resource.schoolId` is in the schema, but no School write path has ever been built — who at a school may publish resources is an undecided product question.

### School-owned `Opportunity.applyUrl` has no write-time URL validation ⚠️
The Organization opportunity routes now accept only `http(s)` apply links (2026-09-24), but `POST`/`PATCH /api/schools/[id]/opportunities(/[opportunityId])` were deliberately left unchanged by that Organization-focused work, so a School Admin can still store a non-link value. **Not a public-safety hole**: `/opportunities` renders every `applyUrl` through `safeHttpHref()`, so a non-`http(s)` value simply displays without a link. Adding the same `parseOptionalHttpUrl()` check to the two School routes is a small, additive follow-up — it would begin rejecting inputs like a bare `www.example.com` that are accepted (as dead links) today.

### `POST /api/auth/register-organization` has no UI caller ⚠️
Organization registration in the UI goes through `/register?role=ORGANIZATION_ADMIN` (`/api/auth/register`) and then `CreateOrgPrompt` → `POST /api/organizations/create-for-admin`. `/api/auth/register-organization` is still a live, unauthenticated endpoint that creates a user and an organization in one step, with nothing in the app calling it. It now validates `website` like every other Organization write path, but whether it should remain at all is an open decision — removing a working endpoint is outside what a hardening change may do on its own.
