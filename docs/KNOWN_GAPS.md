...
### PostgreSQL findings (F1–F7)

Recorded during the PostgreSQL block. **F1 and F2 are fixed on branch `pg-foundation` (F1 in PG-F1, 2026-09-25; F2's rules A6 and A5, then A1–A4, on 2026-09-26); F3–F7 are not fixed.** Proposed fixes are proposals only — each needs its own approval. Evidence lives outside the repository under `C:\MEGA_DB_Backup\PG-KM9`, `PG-KM10`, `PG-F1`, `PG-F2`, `PG-F2FIX` and `PG-F2FIX2` on the development machine (see [TESTING.md](TESTING.md)).
