...
- **`Organization.academyParticipant`** — a self-service boolean, independent of `Organization.verified`. Publishing a course (`PATCH /api/courses/[courseId]`, `published: true`) and enrolling (`POST /api/courses/[courseId]/enroll`) both require **both** `verified` and `academyParticipant` to be `true`; enrollment additionally requires `isActive` (since 2026-09-24), matching the read-time visibility condition below. Toggled by the Organization Admin (`PATCH /api/organizations/[id]` — which since 2026-09-24 also accepts the profile fields `description` and `website`, never `name`/`slug`), checked fresh on every request, never cached; turning it off blocks new publish/enroll attempts immediately and has zero effect on existing courses, enrollments, or certificates.
- **Global read-time visibility**: `/courses`, `/courses/[slug]`, the homepage's course query, and (since 2026-09-24) `/approaches/[slug]`'s course list and `/approaches`' counts all key off the identical `published && organization.verified && organization.academyParticipant && organization.isActive` condition — a course disappears from every one of these surfaces the instant any one fact goes false, and reappears the instant it's true again, with the course row/enrollments/certificates never touched. `/courses/[slug]/learn` (an already-enrolled learner's own access) is deliberately exempt from this gate — historical access persists regardless of the organization's current state.
...
- **Public owner-eligibility & link safety (2026-09-24)** — every public listing of Opportunities/Resources (`/opportunities`, homepage, `/resources`, `/approaches/[slug]`) requires the owning School or Organization to be `verified && isActive` (`eligibleContentOwnerWhere()`, `src/lib/publicVisibility.ts`); `/organizations` now also requires `isActive` and supports `?q=`/`?academy=1`. Public links built from user input (`Organization.website`, `Opportunity.applyUrl`) are validated as `http(s)` on every Organization write path (`parseOptionalHttpUrl()`) and rendered only through `safeHttpHref()` (`src/lib/safeUrl.ts`). Full detail: [ORGANIZATION_INSTITUTIONAL_CONTEXT.md](ORGANIZATION_INSTITUTIONAL_CONTEXT.md).
