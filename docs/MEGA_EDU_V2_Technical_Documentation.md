...
- **Engineering**: no automated tests; nothing deployed; PostgreSQL tested in development only, findings F1–F8 resolved on `pg-foundation`; local-filesystem uploads.
...
- **Engineering** — object storage adapter in `uploads.ts`; PostgreSQL: bulk routes fixed (PG-KM2) and development database moved (PG-KM3–PG-KM10, branch `pg-foundation`); next — the merge (findings F1–F8 resolved), hosting (D3); an automated test harness.
