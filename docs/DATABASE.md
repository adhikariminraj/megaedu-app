...
**Two database-enforced partial unique indexes** (migration `1_integrity_partial_indexes`; Prisma 5.20 cannot express them in `schema.prisma`, so they exist only in the migration): `AcademicSession_one_active_per_school` (`"schoolId"` WHERE `status = 'ACTIVE'`) and `StudentSchoolAffiliation_one_open_per_student` (`"studentId"` WHERE `status IN ('ACTIVE','PENDING')`). Every future migration must be reviewed so it never drops them (decision D5).
...
**Notes**: ⚠️ the `@@unique` above reliably catches a duplicate *section-specific* slot but, on its own, does **not** catch a second *grade-wide* row for the same grade/session (`NULL ≠ NULL` in a unique index, same caveat as `TeacherAcademicAssignment`'s overlap rule) — this was found via a live duplicate-creation test during Phase 3B and fixed with an explicit app-level pre-check inside the create route's transaction, re-verified in both a single-request and an in-batch scenario. ⚠️ That pre-check relied on SQLite running one write at a time. On PostgreSQL it is **not** safe under simultaneous requests: in PG-KM9, 10 simultaneous requests for one free grade-wide slot all succeeded and created 10 rows (finding **F1**, [KNOWN_GAPS.md](KNOWN_GAPS.md#postgresql-findings-f1f7); test data only, removed afterwards; not fixed).
