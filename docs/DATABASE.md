...
**Datasource** (branch `pg-foundation`, not yet merged into `main`): **PostgreSQL** — development database `megaedu_dev` on a local PostgreSQL 18, UTF-8 with `C` collation; schema applied from the reviewed migrations in `prisma/migrations/` (`0_init` through `5_f8_offering_restrict`), never `db push`. Staging/production are not configured yet (decision D3 — see [DEPLOYMENT.md](DEPLOYMENT.md)). `main` still uses SQLite (`prisma/dev.db`), which is preserved unchanged as the rollback source. **No Prisma `enum`s are used anywhere** — originally because SQLite's connector doesn't support them; the plain-`String` convention stays by choice — every status/type/role field is a plain `String`, with valid values documented in a comment above the field.

**Nine database-enforced partial unique indexes** (migrations `1_integrity_partial_indexes`, `2_class_teacher_grade_wide_unique`, `3_f2_annual_and_default_unique` and `4_f2_a1_to_a4_unique`; Prisma 5.20 cannot express them in `schema.prisma`, so they exist only in the migrations): `AcademicSession_one_active_per_school` (`"schoolId"` WHERE `status = 'ACTIVE'`), `StudentSchoolAffiliation_one_open_per_student` (`"studentId"` WHERE `status IN ('ACTIVE','PENDING')`), since PG-F1, `ClassTeacherAssignment_one_grade_wide_per_session` (`"schoolGradeId", "academicSessionId"` WHERE `"sectionId" IS NULL` — finding F1), since the F2 fix (rules A6, A5), `CoScholasticResult_one_annual_per_student_area_session` (`"studentId", "areaId", "academicSessionId"` WHERE `"coScholasticPeriodId" IS NULL`) and `AssessmentFrameworkAssignment_one_default_per_grade_session` (`"academicSessionId", "schoolGradeId"` WHERE `"gradeSubjectId" IS NULL`), and, since the same fix's rules A1–A4, `TeacherAcademicAssignment_one_grade_wide_per_teacher_subject` (`"teacherId", "academicSessionId", "schoolGradeId", "subjectId"` WHERE `"sectionId" IS NULL`), `TeachingPlan_one_grade_wide_per_grade_subject` (`"gradeSubjectId"` WHERE `"sectionId" IS NULL`), `StudentEvaluation_one_general_per_student_teacher_session` (`"studentId", "teacherId", "academicSessionId"` WHERE `"gradeSubjectId" IS NULL`) and `AssessmentComponent_unique_name_framework_level` (`"frameworkId", "name"` WHERE `"periodId" IS NULL`). The last seven were each approved as an explicit exception to decision D7. Every future migration must be reviewed so it never drops them (decision D5).
...
**Notes**: unlike `Teacher`/`Student`, `Parent.userId` is **required** (`String @unique`, not nullable) — a Parent identity cannot exist without a login account. Parent has no `TeacherSchoolAffiliation`/`StudentSchoolAffiliation`-equivalent of its own; a Parent's institutional context is entirely indirect, through their linked children's own school relationships. `ParentStudent.confirmedAt` (nullable `DateTime`, added by the Parent-Student Linking Trust Boundary kilometer) is the only relationship state: `null` means a request exists but grants no access; a real timestamp means confirmed, with full Parent access. No status enum, no history — a decline or unlink is always a hard delete of the row. See [PARENT_STUDENT_LINKING.md](PARENT_STUDENT_LINKING.md).
...
**Constraints**: an `ENDED` row for the same (person, school) pair must remain legal (that's REJOIN), so there is no plain unique constraint. **Student side**: at most one open (`ACTIVE`/`PENDING`) affiliation per student, at any school, is enforced by the partial unique index `StudentSchoolAffiliation_one_open_per_student` as well as by `createStudentAffiliation()`'s own check (a simultaneous duplicate becomes an `AffiliationError` → `409`, shown under real concurrency in PG-KM9). **Teacher side**: the open-affiliation-per-school rule remains application-level (`src/lib/affiliation.ts`), since teachers may legitimately hold several open affiliations.
...
**Constraints**: one `ACTIVE` session per school — enforced by `transitionAcademicSession()` **and**, on PostgreSQL, by the partial unique index `AcademicSession_one_active_per_school` (under the former SQLite + `db push` setup this was application-level only, since a partial unique index cannot be expressed in `schema.prisma`). A simultaneous duplicate becomes a clean `409` (shown under real concurrency in PG-KM9).
...
**Delete behavior**: cascades from `SchoolGrade`/`Subject` (no route deletes either). A real `DELETE` route exists (`.../grades/[schoolGradeId]/subjects/[gradeSubjectId]`), but only an **empty** offering can be removed (finding F8): every foreign key to `GradeSubject` is `RESTRICT` — `TeachingPlan`, `TeachingUnit`, `Homework`, `StudentEvaluation`, `ParentTeacherMeeting` and `AssessmentFrameworkAssignment` since migration `5_f8_offering_restrict`, `TeacherAcademicAssignment`, `AssessmentComponentResult` and `AssessmentResultPublication` from the start — and the route answers `409` with a list of what depends on it, and `409` for a closed session's offering. The Phase 3A reasoning that "nothing permanent points at a `GradeSubject` row, so a hard delete is safe" stopped being true once later phases attached units and tests, homework, evaluations, meetings, framework overrides and results to it.
...
**Constraints**: `@@unique([teacherId, academicSessionId, schoolGradeId, sectionId, subjectId])` — catches an exact duplicate reliably (SQL unique indexes work normally when `sectionId` is a real value). Does **not**, by itself, prevent two grade-wide (`sectionId: null`) rows for the same tuple — SQL treats `NULL ≠ NULL` in unique indexes — so since the F2 fix (rule A1, migration `4_f2_a1_to_a4_unique`) the partial unique index `TeacherAcademicAssignment_one_grade_wide_per_teacher_subject` blocks a second grade-wide row. The grade-wide/section-specific overlap rule spans different rows, which no index can express, so it is enforced in the route — inside a `SERIALIZABLE` transaction since the same fix, because the check alone was not safe under simultaneous requests on PostgreSQL (finding **F2**, [KNOWN_GAPS.md](KNOWN_GAPS.md#postgresql-findings-f1f8)). A request that loses to a simultaneous one gets `409` and its whole batch rolls back.
**Delete behavior**: cascades from `Teacher`; blocks deletion of its `AcademicSession`, `SchoolGrade`, `Subject` and `GradeSubject` (`RESTRICT`); a real `DELETE` route exists (`.../teacher-academic-assignments/[assignmentId]`) — not audited, same as `TeacherGradeAssignment`'s own delete route.
...
**Notes**: ⚠️ the `@@unique` above reliably catches a duplicate *section-specific* slot but, on its own, does **not** catch a second *grade-wide* row for the same grade/session (`NULL ≠ NULL` in a unique index, same caveat as `TeacherAcademicAssignment`'s overlap rule) — this was found via a live duplicate-creation test during Phase 3B and fixed with an explicit app-level pre-check inside the create route's transaction, re-verified in both a single-request and an in-batch scenario. ⚠️ That pre-check relied on SQLite running one write at a time. On PostgreSQL it is **not** safe under simultaneous requests: in PG-KM9, 10 simultaneous requests for one free grade-wide slot all succeeded and created 10 rows (finding **F1**, [KNOWN_GAPS.md](KNOWN_GAPS.md#postgresql-findings-f1f8); test data only, removed afterwards). ✅ **Fixed on branch `pg-foundation` (PG-F1, 2026-09-25)**: migration `2_class_teacher_grade_wide_unique` adds the partial unique index `ClassTeacherAssignment_one_grade_wide_per_session` on `("schoolGradeId", "academicSessionId") WHERE "sectionId" IS NULL` — at most one grade-wide row (Grade Coordinator) per grade per session, in every session; section rows are unaffected. The route's pre-check stays for the normal "already filled → skipped" response; a simultaneous request that loses the race now gets `409` and its whole batch rolls back. Verified: the PG-KM9 race reproduced on a restored copy (10 rows) gave exactly 1 row with 9 rejections after the migration; through the app, 3 simultaneous losing requests got `409` with nothing from their batches left behind. Not on `main` (SQLite).
...
**Constraints**: `@@unique([gradeSubjectId, sectionId])` — at most one plan per grade-subject-and-section scope. It catches a duplicate section plan; two grade-wide (`sectionId: null`) plans are blocked by the partial unique index `TeachingPlan_one_grade_wide_per_grade_subject` (F2 fix, rule A2, migration `4_f2_a1_to_a4_unique`), since `NULL ≠ NULL` in the `@@unique`.
**Delete behavior**: blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, it cascaded — finding F8); no delete route — the creation route is find-or-update-else-create, not a bare insert (same NULL-uniqueness reasoning as above), so a second "create" call updates the existing plan instead; when simultaneous saves collide, the one whose insert is rejected updates the plan the other created (no ordering between simultaneous saves is defined).
...
**Delete behavior**: blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, it cascaded, taking its unit tests and their results with it — finding F8); no delete route.
...
**Key fields**: `id, academicSessionId (FK), schoolGradeId (FK), sectionId? (FK to Section), gradeSubjectId (FK), subjectId (FK), teacherId (FK), title, instructions, dueDate, status (default "DRAFT"), publishedAt?, targetStudentId? (FK to Student, K1), createdAt, updatedAt`. Valid `status`: `DRAFT | PUBLISHED`.
**Constraints**: none beyond FKs — a grade/section/subject may have any number of Homework rows, no uniqueness restriction.
**Delete behavior**: blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, it cascaded, taking its applicability, completions, completion audit, submissions and reviews with it — finding F8); no delete route in this phase.
**Notes**: deliberately **not** fanned out into a per-student row itself the way `UnitTestResult` is — that role is now played by the separate `HomeworkApplicability` model (K1, below), never a column on `Homework` directly. `sectionId: null` = grade-wide (every section); a real value = that section only — the same three-way idiom already used by `TeachingUnit`/`StudentEvaluation` via `sectionScopeWhere()`. `teacherId` references the `Teacher` identity directly (matching `StudentEvaluation`'s precedent), not just a `createdByUserId` audit field the way `TeachingUnit`/`UnitTest` do — deliberate, so authorship resolves through the institutional Teacher identity rather than the raw account. `dueDate` is date-only, following the `Attendance.date`/`UnitTest.testDate` convention (always derived from a client-supplied `"YYYY-MM-DD"` string, never a server-computed "today"). `status` is a single gate for both Student and Parent visibility simultaneously — not two independent flags like `StudentEvaluation.visibleToParent`/`visibleToStudent`. Once `PUBLISHED`, `title`/`instructions`/`dueDate`/`sectionId` are frozen — matching the "permanent once shared" precedent already established for `StudentEvaluation` sharing and `Certificate` issuance. `targetStudentId` (K1): `null` = Regular Homework (grade/section-scoped, as above); set = Individual Homework, targeting exactly this student — `sectionId` is forced `null` in that case, since section is irrelevant to who an Individual Homework applies to.

### `HomeworkApplicability` (K1)
**Purpose**: the sole, canonical Student ↔ Homework relationship — "does this homework apply to this student." **Currently used**: yes.
**Key fields**: `id, homeworkId (FK, cascade), studentId (FK, cascade), assignedAt`.
...
**Constraints**: `@@unique([studentId, teacherId, academicSessionId, gradeSubjectId])` — reliably catches an exact duplicate subject-specific slot; does **not**, by itself, catch a second general (`gradeSubjectId: null`) evaluation from the same teacher/student/session (the familiar `NULL ≠ NULL` unique-index gap already seen with `TeacherAcademicAssignment`/`ClassTeacherAssignment`) — the create route pre-checks this case explicitly, and since the F2 fix (rule A3, migration `4_f2_a1_to_a4_unique`) the partial unique index `StudentEvaluation_one_general_per_student_teacher_session` enforces it in the database; a simultaneous duplicate, general or subject, gets the same `409` as the pre-check.
**Delete behavior**: cascades from `Student`; blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, removing the offering turned a subject evaluation into a general one — finding F8); no delete route — remarks are edited in place via `updateEvaluationRemarks()`, never deleted and recreated.
...
**Delete behavior**: cascades from `Student`; blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, removing the offering turned a subject meeting into a general one — finding F8); no delete route — a meeting is cancelled (`status: "CANCELLED"`), never deleted.
...
**Constraints**: `@@unique([frameworkId, periodId, name])` — ⚠️ **NULL≠NULL gap**: reliably blocks a duplicate within the same real period, but not two identically-named components both with `periodId: null`. Pre-checked explicitly via `componentCollisionExists()` (`src/lib/assessmentFramework.ts`) before every create and rename. Since the F2 fix (rule A4, migration `4_f2_a1_to_a4_unique`) the partial unique index `AssessmentComponent_unique_name_framework_level` enforces the framework-level case in the database; the add and rename routes return `409` for a pre-check hit and for a simultaneous duplicate alike.
...
**Constraints**: `@@unique([academicSessionId, schoolGradeId, gradeSubjectId])` — reliably blocks a duplicate subject-specific override; on its own it would not block a second grade-default (`gradeSubjectId: null`) assignment for the same grade/session (NULL≠NULL, the same recurring gap as `TeacherAcademicAssignment`, `ClassTeacherAssignment` and `StudentEvaluation`). A grade default is pre-checked via `assignmentCollisionExists()` and, since the F2 fix (migration `3_f2_annual_and_default_unique`), also enforced by the partial unique index `AssessmentFrameworkAssignment_one_default_per_grade_session`; the create route returns `409` for a pre-check hit and for a simultaneous duplicate alike (finding F2, rule A5).
**Delete behavior**: no cascade dependents; blocks removal of its `GradeSubject` (`RESTRICT` since migration `5_f8_offering_restrict`; before, removing the offering turned a subject override into the grade default — finding F8); a real `DELETE` route exists — not audited, current-state config, same as `TeacherAcademicAssignment`'s own `DELETE` route.
