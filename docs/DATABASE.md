...
**Datasource** (branch `pg-foundation`, not yet merged into `main`): **PostgreSQL** — development database `megaedu_dev` on a local PostgreSQL 18, UTF-8 with `C` collation; schema applied from the reviewed migrations in `prisma/migrations/` (`0_init` + `1_integrity_partial_indexes`), never `db push`. Staging/production are not configured yet (decision D3 — see [DEPLOYMENT.md](DEPLOYMENT.md)). `main` still uses SQLite (`prisma/dev.db`), which is preserved unchanged as the rollback source. **No Prisma `enum`s are used anywhere** — originally because SQLite's connector doesn't support them; the plain-`String` convention stays by choice — every status/type/role field is a plain `String`, with valid values documented in a comment above the field.

**Five database-enforced partial unique indexes** (migrations `1_integrity_partial_indexes`, `2_class_teacher_grade_wide_unique` and `3_f2_annual_and_default_unique`; Prisma 5.20 cannot express them in `schema.prisma`, so they exist only in the migrations): `AcademicSession_one_active_per_school` (`"schoolId"` WHERE `status = 'ACTIVE'`), `StudentSchoolAffiliation_one_open_per_student` (`"studentId"` WHERE `status IN ('ACTIVE','PENDING')`), since PG-F1, `ClassTeacherAssignment_one_grade_wide_per_session` (`"schoolGradeId", "academicSessionId"` WHERE `"sectionId" IS NULL` — finding F1) and, since the F2 fix (rules A6, A5), `CoScholasticResult_one_annual_per_student_area_session` (`"studentId", "areaId", "academicSessionId"` WHERE `"coScholasticPeriodId" IS NULL`) and `AssessmentFrameworkAssignment_one_default_per_grade_session` (`"academicSessionId", "schoolGradeId"` WHERE `"gradeSubjectId" IS NULL`). The last three were each approved as an explicit exception to decision D7. Every future migration must be reviewed so it never drops them (decision D5).
