...
**Three database-enforced partial unique indexes** (migrations `1_integrity_partial_indexes` and `2_class_teacher_grade_wide_unique`; Prisma 5.20 cannot express them in `schema.prisma`, so they exist only in the migrations): `AcademicSession_one_active_per_school` (`"schoolId"` WHERE `status = 'ACTIVE'`), `StudentSchoolAffiliation_one_open_per_student` (`"studentId"` WHERE `status IN ('ACTIVE','PENDING')`) and, since PG-F1, `ClassTeacherAssignment_one_grade_wide_per_session` (`"schoolGradeId", "academicSessionId"` WHERE `"sectionId" IS NULL` — finding F1, approved as an explicit exception to decision D7). Every future migration must be reviewed so it never drops them (decision D5).
