...
**Datasource** (branch `pg-foundation`, not yet merged into `main`): **PostgreSQL** — development database `megaedu_dev` on a local PostgreSQL 18, UTF-8 with `C` collation; schema applied from the reviewed migrations in `prisma/migrations/` (`0_init` through `4_f2_a1_to_a4_unique`), never `db push`. Staging/production are not configured yet (decision D3 — see [DEPLOYMENT.md](DEPLOYMENT.md)). `main` still uses SQLite (`prisma/dev.db`), which is preserved unchanged as the rollback source. **No Prisma `enum`s are used anywhere** — originally because SQLite's connector doesn't support them; the plain-`String` convention stays by choice — every status/type/role field is a plain `String`, with valid values documented in a comment above the field.
...
**Delete behavior**: cascades from `GradeSubject`; no delete route — the creation route is find-or-update-else-create, not a bare insert (same NULL-uniqueness reasoning as above), so a second "create" call updates the existing plan instead; when simultaneous saves collide, the one whose insert is rejected updates the plan the other created (no ordering between simultaneous saves is defined).
