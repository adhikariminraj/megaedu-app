...
7. Verify: the content fingerprint must equal the manifest's (`MEGA_DB_Backup\PG-DR\pgdr-db.js check-state` in the set also checks migrations, partial indexes and roles), the integrity suite (`MEGA_DB_Backup\PG-F2FIX2\pgf2fix2-pgkm7-verify-db.js` for sets taken after the F2 fix; older sets hold the suite matching their own migrations), and `db:verify:demo` (18 of 18; `DATABASE_URL` set for that process only — see the README).
