...
- **Teacher Qualitative Evaluation & Parent-Teacher Meetings ("Phase 3C-1")** — a teacher's narrative evaluation of a student (`StudentEvaluation`), General (Class/Section Teacher) or Subject-specific (Subject Teacher), distinguished by one nullable field, not a type field or two systems; freely editable while private, audited via `StudentEvaluationAudit` from the moment it's shared with either a Parent or a Student — two fully independent visibility gates. Parent-Teacher Meetings (`ParentTeacherMeeting`) support both periodic (bulk) and occasional (single) scheduling through the same route, School-Admin/Teacher-initiated only, Parent-visible only (Students have no PTM visibility in this phase — a structural guarantee, not a hidden UI section), with an optional link from a meeting to a prepared evaluation. See [ASSESSMENT_AND_EVALUATION.md](ASSESSMENT_AND_EVALUATION.md).
