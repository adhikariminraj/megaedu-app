...
- **`GradeSubject`** — which subjects a grade offers, **for one specific academic session**. Unlike `SchoolGrade`/`Section`, this is not reusable config: every session starts with zero rows, and nothing is ever auto-copied from the prior session. A real `DELETE` route exists (unlike `Subject`/`Section`), but only an **empty** offering can be removed: anything recorded against it (teacher assignments, plans, units and tests, homework, evaluations, meetings, framework overrides, results) blocks removal with a `409` that lists it, a closed session's offering is never removed, and every foreign key to it is `RESTRICT` (finding F8).
...
3. **Overlap rule** (checked in the route — SQL unique indexes treat `NULL ≠ NULL`, so a plain `@@unique` cannot catch two grade-wide rows colliding, and no index can express grade-wide vs section-specific): the *same* teacher may never hold both a grade-wide and a section-specific row for the same `(teacherId, academicSessionId, schoolGradeId, subjectId)` tuple. Requesting grade-wide is rejected if *any* row already exists for that tuple; requesting section-specific is rejected only if a grade-wide row already exists for it — other sections for the same teacher/subject are unaffected. Since the F2 fix (rule A1, migration `4_f2_a1_to_a4_unique`) the route's transaction runs at `SERIALIZABLE` isolation, so simultaneous requests cannot together break the rule (PostgreSQL aborts one of them), and a second grade-wide row is also blocked by the partial unique index `TeacherAcademicAssignment_one_grade_wide_per_teacher_subject`; either way the whole batch rolls back and the route returns `409` ("please refresh and try again").
